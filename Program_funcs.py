...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  1 15:42:33 2019
...
def tree_to_prefix(program_tree):
    output = []
    def helper(cur):
        output.append({'function': cur['function'],
                       'value_inputs': [x for x in cur['value_inputs']]})
...
def tree_to_list(program_tree):
    ...
    def helper(cur, idx):
        output[idx] = {'function': cur['function'],
              'value_inputs': [x for x in cur['value_inputs']],
              'inputs':[]}
        next_idx = idx - 1
...
def list_to_postfix(program_list):
    return tree_to_postfix(prefix_to_tree(program_list))
